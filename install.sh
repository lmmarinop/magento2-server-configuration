...
VARNISH_VERSION=$(awk -F "=" '/varnish_version/ {print $2}' install.ini)
...
if [ $INSTALL_MARIADB == "yes" ];
    then
        echo ""
        echo "INSTALLATION AND CONFIGURATION: MARIADB DATABASE AND SERVER"
        sleep $WAIT
...
if [ $CONFIGURE_CERTBOT_CERTIFICATES == "yes" ];
    then
        ...
        sleep $WAIT
        bash install/certbot_certificate.sh $DOMAIN $ADMIN_EMAIL
fi;
...
if [ $VARNISH_NGINX_SSL == "yes" ];
    then
        ...
        sleep $WAIT
        bash install/varnish_ssl.sh $DOMAIN $VARNISH_VERSION
